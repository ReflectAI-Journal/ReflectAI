...
exports.generateCounselorResponse = generateCounselorResponse;
exports.generatePhilosopherResponse = generatePhilosopherResponse;
const openai_adapter_1 = __importStar(require("./openai-adapter"));
const security_1 = require("./security");
// the newest OpenAI model is "gpt-4o" which was released May 13, 2024. do not change this unless explicitly requested by the user
const MODEL = "gpt-4o";
// Sanitize logging to prevent key exposure in logs (only show first 5 chars)
const maskedKey = openai_adapter_1.apiKey.length > 8 ?
    `${openai_adapter_1.apiKey.substring(0, 5)}${'*'.repeat(openai_adapter_1.apiKey.length - 8)}${openai_adapter_1.apiKey.substring(openai_adapter_1.apiKey.length - 3)}` :
    '(not set)';
console.log(`Using OpenAI API key (preview): ${maskedKey}`);
// Validate the API key format
if (openai_adapter_1.apiKey.length > 10) {
    console.log(`API key starts with correct format (sk-): ${openai_adapter_1.apiKey.startsWith('sk-')}`);
    if (!openai_adapter_1.apiKey.startsWith('sk-')) {
...
function generateAIResponse(journalContent) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            (0, security_1.logPrivacyEvent)("ai_content_processed", 0, "Journal content processed for AI analysis");
            const prompt = `
      ...
      ${sanitizedContent}
    `;
            const response = yield openai_adapter_1.default.chat.completions.create({
                model: MODEL,
                messages: [
                    {
                        role: "system",
...
function generateFallbackResponse(journalContent) {
    ...
    positiveWords.forEach(word => {
        ...
        if (lowerContent.includes(`very ${word}`) || lowerContent.includes(`really ${word}`))
            positiveScore += 0.5;
    });
...
function analyzeSentiment(journalContent) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Sanitize content before analysis to remove PII
            const sanitizedContent = (0, security_1.sanitizeContentForAI)(journalContent);
            // Log that content was processed for sentiment analysis
            (0, security_1.logPrivacyEvent)("sentiment_analysis", 0, "Journal content processed for sentiment analysis");
            const response = yield openai_adapter_1.default.chat.completions.create({
                model: MODEL,
                messages: [
                    {
 ...
 */
function generateChatbotResponse(messages_1) {
    return __awaiter(this, arguments, void 0, function* (messages, supportType = 'general', personalityType = 'default', customInstructions) {
        try {
          ...
          Focus on helping them reflect rather than giving long advice.`;
            ...
            const apiMessages = messages.slice(-10).map(msg => {
                ...
                if (msg.role === 'user') {
                    return {
                        role: msg.role,
                        content: (0, security_1.sanitizeContentForAI)(msg.content)
                    };
                }
                return {
                    role: msg.role,
            ...
            (0, security_1.logPrivacyEvent)("chatbot_message_processed", 0, "Chat messages processed for AI response");
        ...
        ${customInstructions}`;
            ...
            if ((error === null || error === void 0 ? void 0 : error.message) && (error.message.includes("exceeded your current quota") ||
...
function generateChatbotResponseFallback(messages, supportType, personalityType = 'default', customInstructions) {
    ...
    const lastUserMessage = ((_a = messages
        .filter(msg => msg.role === 'user')
        .slice(-1)[0]) === null || _a === void 0 ? void 0 : _a.content) || '';
    ...
    function christianPersonality(response) {
    ...
    function empatheticListenerPersonality(response) {
    ...
    function solutionFocusedPersonality(response) {
    ...
    function traumaInformedPersonality(response) {
        const safetyPhrases = [
    ...
    function mindfulnessBasedPersonality(response) {
    ...
    function cognitiveBehavioralPersonality(response) {
        const cbtPhrases = [
    ...
    function strengthBasedPersonality(response) {
        const strengthPhrases = [
    ...
    function holisticWellnessPersonality(response) {
...
function generateCounselorResponse(content) {
    return __awaiter(this, void 0, void 0, function* () {
        const response = yield openai_adapter_1.default.chat.completions.create({
            model: MODEL,
            messages: [
...
function generatePhilosopherResponse(content) {
    return __awaiter(this, void 0, void 0, function* () {
        const response = yield openai_adapter_1.default.chat.completions.create({
            model: MODEL,
            messages: [
